...
// Top-level constants (accessible by JSX components below)
const YELLOW_PAGES_COVERAGE_NOTE =
  "Yellow Pages data coverage varies by location and sector.";
const YELP_COVERAGE_NOTE =
  "Yelp data coverage varies by location and sector.";

// Safe inline fallback for SourceTooltip component
interface SourceTooltipProps {
  text?: string;
  children?: React.ReactNode;
}

const SourceTooltip: React.FC<SourceTooltipProps> = ({ text, children }) => {
  return (
    <div title={text} className="inline-block">
      {children}
    </div>
  );
};
    ...
    <form onSubmit={handleStartScraping} className="space-y-4">
      <div>
        ...
        <div className="flex flex-wrap gap-4 text-sm font-medium text-slate-700">
          ...
          <SourceTooltip text={YELP_COVERAGE_NOTE}>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                name="yelp"
          ...
          <SourceTooltip text={YELLOW_PAGES_COVERAGE_NOTE}>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                name="yellowpages"
